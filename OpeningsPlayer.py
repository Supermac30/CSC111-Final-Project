...
class ReversiOpeningsGameTree(GameTree):
    ...
    def __init__(self, start_state: Reversi.ReversiGameState, initialise_tree: bool = True) -> None:
    ...
    def add_move_sequence(self, moves: list[Tuple[int, int]]) -> None:
        ...
        if moves[0] in possible_moves:
            ...
            if not new_move.make_move(moves[0], True):
                breakpoint()
            child = ReversiOpeningsGameTree(new_move, initialise_tree=False)
            self.children.append(child)

        child.add_move_sequence(moves[1:])

    def expand_tree(self, state: GameState) -> None:
        """Doesn't expand the tree with legal moves, sticking with the moves in the opening.
        """
        return None
